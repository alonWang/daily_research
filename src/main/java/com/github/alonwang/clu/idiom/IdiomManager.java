...
public class IdiomManager {
    ...
    public static void init() {
        ...
        idioms = Arrays.stream(new String(raws).split("\r\n")).parallel().filter(w -> w.length() == 4).collect(Collectors.toCollection(ConcurrentHashSet::new));
        headPinyins = idioms.parallelStream().collect(Collectors.groupingBy((String s) -> Pinyin.toPinyin(s.charAt(0)), Collectors.toCollection(ConcurrentHashSet::new)));
        current = next(null);
        idle = true;
    }
    ...
    public static String next(String prev) {
        if (prev == null) {
            current = idioms.stream().skip(ThreadLocalRandom.current().nextInt(20)).findAny().orElse("没成语了");
